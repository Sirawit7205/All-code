...
void calcRank(double mean,double sd)
{
	double rank[5][5]={};
	rank[0][0]=mean+sd; 
	rank[0][1]=100;
...
void displayScore(int score[], double mean, double sd, char name[][20])
{
	int i;
	for(i=0;i<10;i++)
	{
		...
		if(score[i]>=mean+sd) 
			printf("A\n");
		else if(score[i]>=mean+(0.5*sd)) 
			printf("B\n");
		else if(score[i]>=mean-sd) 
			printf("C\n");
		else if(score[i]>=mean-(2*sd)) 
			printf("D\n");
		else 
			printf("F\n");
	}
...
int main()
{
	...
	calcRank(mean,sd);
	displayScore(score,mean,sd,name);
	return 0;
}